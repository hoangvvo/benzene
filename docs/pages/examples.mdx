  ...
  return (
    <>
      <input
        ...
        type="search"
        class="appearance-none border rounded py-2 px-3 leading-tight focus:outline-none focus:ring w-full"
        placeholder="Search"
      />
      <div style={{ height: "32rem", overflow: "auto" }}>
        {searchedExamples.map((example) => (
          <Callout key={example.slug} emoji="">
            ...
            <div>
              <a
                href={`http://githubbox.com/${repo}/tree/main/examples/${example.slug}`}
              >
                Source
              </a>{" "}
              <a
                href={`https://github.com/${repo}/tree/main/examples/${example.slug}`}
              >
                CodeSandbox
              </a>
            </div>
