...
Check out the [source code](https://github.com/hoangvvo/benzene/tree/main/examples/benzene-votes) or follow along.
...
await fetch("/graphql", {
  ...
  body: JSON.stringify({
    query: "query { posts { id, title, author { name }, votes } }",
  }),
}).then((res) => res.json());
```

Before we make a mutation, let's try to subscribe to our posts' changes using GraphQL Subscription over WebSockets.
...
Let's try to subscribe to `postSubscribe`.
...
websocket.send(
  JSON.stringify({
    type: "subscribe",
    id: "1",
    payload: { query: "subscription { postSubscribe { title, votes } }" },
  })
);
```
...
await fetch("/graphql", {
  method: "POST",
  headers: { "content-type": "application/json" },
  body: JSON.stringify({ query: "mutation { postUpvote(postId: 1) { id, votes } }" }),
}).then((res) => res.json());
```

We can also notice a message coming from the WebSocket channel:
...
```json
{
  ...
  "payload": {
    "data": {
      "postSubscribe": { "title": "Introduction to GraphQL", "votes": 5 }
    }
  },
  "type": "next"
}
