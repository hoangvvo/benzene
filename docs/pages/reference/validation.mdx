...
<Callout emoji="⚠️">
  Setting custom validation rules can make our implementation incompatible with the GraphQL specs.
  Therefore, it is best to add it in addition to the `specifiedRules` from `graphql-js`.
</Callout>
